# AGENTS.md · Sparta_AX_Project 작업 헌법
...
로그는 선택 사항이 아니라 작업 완료 조건이다. 주요 요청이 끝났는데 로그를 쓰지 않았다면 해당 요청은 완료로 보고하지 않는다.

### 로그 작성 트리거

- 사용자 요청으로 파일·문서·산출물·계획·검증 기준이 바뀌면 반드시 `Worklog.md`를 갱신한다.
- 제출 구조, 파이프라인 방식, 스킬 구조, 데이터 처리 기준, 금지 범위, 검증 방식이 바뀌면 반드시 `Decisionlog.md`를 갱신한다.
- 오류, 실패한 명령, 인코딩 문제, 검증 실패, 문서 간 충돌, 설계 충돌이 실제로 발생하면 반드시 `Troubleshootinglog.md`를 갱신한다.
- 설명만 한 요청이라도 이후 작업 기준이나 사용자의 판단에 영향을 주면 `Worklog.md`에 남긴다.
- 여러 파일을 고친 요청은 최종 보고 전에 `git status --short` 또는 동등한 파일 상태 확인을 수행하고, 로그에 반영한다.
- 로그 작성 여부를 완료 보고의 `기록` 항목에 명시한다.

### 로그 생략 가능 조건

- 단순 상태 조회처럼 파일·방향·기준을 바꾸지 않는 요청은 로그를 생략할 수 있다.
- 로그를 생략할 때도 완료 보고에 “파일 변경 없음으로 로그 미작성”처럼 이유를 남긴다.
