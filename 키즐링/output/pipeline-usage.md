...
5. `kizzling-quality-audit`: 실행 사이클 마지막에 이메일 산출물 QA를 수행한다.
...
`키즐링/scripts/generate_basic_email_drafts.py`는 로컬 검증을 위해 사용한 개발 보조물이며, 최종 제출 파이프라인의 필수 의존성은 아니다.
...
- `키즐링/output/campaign_plan_preview.html`: Challenge 포함 시 캠페인 기획안 시각화
- `키즐링/output/quality_audit_report.md`: 실행 사이클 마지막 품질 QA 결과
...
- Challenge 포함 흐름: `kizzling-analyze-leads → kizzling-message-strategy → kizzling-generate-emails → kizzling-campaign-plan → kizzling-visualize-campaign → kizzling-review-submit → kizzling-quality-audit`
